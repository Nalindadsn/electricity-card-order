...
<?php if (pr($prArray[12])==1) { ?>
          ...
          <li class="nav-item  active shadow">
              ...
              <a class="nav-link active" href="#otherInfosubmenu" data-toggle="collapse" aria-expanded="false" class="dropdown-toggle">
...
<?php if (pr($prArray[13])==1) { ?>
                        ...
                        <li class="nav-item">
                            <a  class="nav-link" href="allOther.php">
                                <i class="fas fa-dot-circle text-primary"></i> සාරාංශය </a>
                            </a>
                        </li>
<?php } ?>
...
<!-- sidenav end -->
  ...
  <div class="main-content">
    ...
    <nav class="navbar navbar-top navbar-expand-md navbar-dark" id="navbar-main">
      <div class="container-fluid">
        <!-- Brand -->
        <a class="h4 mb-0 text-white text-uppercase d-none d-lg-inline-block" href="./index.php">Other Details</a>
        <!-- Form -->
...
<?php if ( pr($prArray[56])==1) { ?>

              <a href="#" class="btn btn-sm btn-neutral" id="modal_button" >කරාමය ඇතුල් කරන්න</a>
<?php } ?>
 ...
 $('#modal_button').click(function(){
  ...
  $(document).on('click', '.update', function(){
    ...
    document.getElementById('m_id').innerHTML='';
        ...
        $('#user_id').val(user_id);
        document.getElementById('m_id').innerHTML=data.tone_name+" <i class='ni ni-bold-right p-1'></i><span class='badge badge-pill badge-primary'>GS Divisions :  "+data.countV+" </span>"+" <i class='ni ni-bold-right'></i><span class='badge badge-pill badge-primary'> BILLING DIVISION :  "+data.countV2+" </span>";
        $('#action').val("Edit");
        $('#operation').val("Edit");
      }
    })
  ...
  $(document).on('click', '.update1', function(){
    var user_id = $(this).attr("id");
    $.ajax({
      url:"admin/fetch_single8_b.php",
      method:"POST",
      data:{user_id:user_id},
      dataType:"json",
      success:function(data)
      {
        $('#tpLbl').text('ග්‍රාම නිලධාරි වසම');
        $('#userModal').modal('show');
        $('#tone_name').val(data.tone_name);
        $('#user_id').val(user_id);
  ...
  $(document).on('click', '.update2', function(){
    var user_id = $(this).attr("id");
    $.ajax({
      url:"admin/fetch_single9_b.php",
      method:"POST",
      data:{user_id:user_id},
      dataType:"json",
      success:function(data)
      {
        $('#tpLbl').text('බිල් කළාපය');
        $('#userModal').modal('show');
        $('#tone_name').val(data.tone_name);
        $('#user_id').val(user_id);
         document.getElementById('m_id').innerHTML=data.parent;
