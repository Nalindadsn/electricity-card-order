...
{
	include('db.php');
	$statement = $pdo->prepare("SELECT * FROM incategory WHERE status=1");
	$statement->execute();
...
$query = '';
$output = array();
$query .= 'SELECT * FROM incategory WHERE status=1 ';
...
if ( pr($prArray[70])==1) {
	if ($row["id"]==1) {
	$sub_array[] = '<button type="button"  class="btn btn-danger btn-sm " disabled>Deactivate</button><span class="text-danget">මෙය අක්‍රීය කළ නොහැක.</span>';
	}else{
