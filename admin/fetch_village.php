...
{
	include('db.php');
	$statement = $pdo->prepare("SELECT * FROM tthree ");
	$statement->execute();
...
$query = '';
$output = array();
$query .= 'SELECT * FROM tthree WHERE  ';
...
foreach($result as $row)
{
	...
	$sub_array = array();
	 ...
	 $sub_array[] = $row["tthree_name"].'<span class="badge badge-pill badge-primary ml-4">'.parentV($row["ttwo_id"]).'</span>';
	 // if ($row['status']==0) {
