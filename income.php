...
<?php if (pr($prArray[25])==1) { ?>
                        ...
                        <li class="nav-item">
                            <a  class="nav-link" href="income.php">
                                <i class="fas fa-dot-circle text-primary"></i>  ආදායම් </a>
                            </a>
                        </li>
<?php } ?>
...
?>
  ...
  <div class="row">
    <div class="col">
      <input type="number"  name="noUnit" id="noUnit" class="form-control"  placeholder="ඒකක ප්‍රමානය *">
                <div id="msg_unit" style=""></div>
    </div>
    <div class="col">  
    <div class="row">
        <div class="col">
        ...
        <div class="col">
      <input type="number"  name="acc_typeN" id="acc_typeN" class="form-control" readonly="readonly" />
...
$("#noUnit").blur(function(e) {
    var uname = $("#category").val();
    var uNV=$("#noUnit").val()
    if (uname == "")
    {
        $("#msg_unit").html("Please fill account number before add this");
    }
    else
    {
        ...
        $.ajax({
            url: "ajaxfile_unit.php",
            data: {username: uNV,a_id:uname},
            type: "POST",
            dataType:"json",
            success: function(data) {
console.log(data)
               //var a=parseInt()
                    $("#last_name").val(data.num)
                    let text=''
if (data.status===0) {
    text+='<span">Monthly Plan is empty</span><a class="ml-4" href="monthPlan.php?id='+data.typeId+'">update</a>'

}else{
if (uNV==='') {text+='<span>...</span>'}else{


                     text+='<table  class="table align-items-center table-dark">'
                      ...
                      text+='<tr>'
                      text+='<thead>'
                     data.arrData.forEach(value => {
                      text+='<tr>'
                    text+='<td>'+value.les+'</td>'
                    text+='<td>'+value.max+'</td>'
                    text+='<td>'+value.points+'*'+value.val+'</td>'
                    text+='<td align="right">'+value.rowam.toFixed(2)+'</td>'
                      text+='</tr>'
                    })

                      text+='<tr>'
                        text+='<td colspan="2">Greater Than '+data.defAmMax+'</td>'
                        text+='<td>'+data.exc+"*"+data.defAmUni+'</td>'
                        text+='<td align="right">'+data.defAm.toFixed(2)+'</td>'
                      text+='</tr>'
                     text+='</table>'
...
"lengthMenu":[[10,25,50,-1],[10,25,50,"All"]],
  ...
  $(document).on('submit', '#user_form', function(event){
    ...
    if(lastName != '' && noUnit != '' && acc_typeN != '')
    {
      $.ajax({
        url:"admin/insert10.php",
        method:'POST',
