...
<?php if (pr($prArray[20])==1) { ?>
          <li class="nav-item  active ">
              ...
              <a class="nav-link" href="#reportSub" data-toggle="collapse" aria-expanded="false" class="dropdown-toggle">
                    ...
                    <ul class="collapse show list-unstyled mx-3" id="reportSub" >
...
<?php if (pr($prArray[21])==1) { ?>
                        ...
                        <li class="nav-item">
                            <a  class="nav-link" href="incomeReport.php">
                                <i class="fas fa-dot-circle text-primary"></i> ආදායම් වාර්තා </a>
                            </a>
                        </li>
<?php } ?>
...
<!-- sidenav end -->
  ...
  <div class="main-content">
    ...
    <nav class="navbar navbar-top navbar-expand-md navbar-dark" id="navbar-main">
      <div class="container-fluid">
        <!-- Brand -->
        <a class="h4 mb-0 text-white text-uppercase d-none d-lg-inline-block" href="./index.php">Income Report <?php echo $_GET['year']."/".$_GET['day'] ?></a>
        <!-- Form -->
        ...
        <div class="header-body">
          <div class="row align-items-center py-4">
            ...
            <div class="col-lg-6 col-5 text-right">
             <!--  <a id="add_button" class="btn btn-sm btn-neutral">Add Expenses</a> -->
        ...
        <div class="col">
          <div class="card shadow">
            <div class="card-header bg-transparent">
              <h3 class="mb-0">ආදායම් වාර්තාව <span class="text-primary"> <?php echo $_GET['year']."-".$_GET['day'] ?></span> </h3>
            </div>
...
<div id="erm"></div>
    <div class="">
<nav class="navbar navbar-expand-lg p-1  navbar-dark bg-primary" style="overflow: auto;">
...
if (date('Y')==$v['year']) {
    ?>
        ...
        <li class="nav-item  ">
          <a class="nav-link active" href="incomeReport.php?year=<?php 
           echo $v['year']; ?>"><?php 
           echo $v['year']; ?></a>
        </li>
    <?php
...
?>
        <li class="nav-item">
          <a class="nav-link" href="incomeReport.php?year=<?php 
           echo $v['year']; ?>"><?php 
           echo $v['year']; ?></a>
        </li>
<?php
...
<nav class="navbar navbar-expand-lg p-1  navbar-dark bg-dark" style="overflow: auto;">
  <div class="container-fluid " style="padding-left: 0 !important;">
    <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNavDropdown" aria-controls="navbarNavDropdown" aria-expanded="false" aria-label="Toggle navigation">
      <span class="navbar-toggler-icon"></span>
    </button>
...
if (isset($_GET['year'])) {
  # code...
   ...
   foreach (incomeCatM($_GET['year'],$_GET['day']) as $key) {
     echo "<li>".getInCatR($key['cat_id']).'<span class="badge bg-dark text-white ml-3">LKR '.$key['amount'].'</span>'."</li>";
   }
}else{
   foreach (incomeCatM(date('Y'),date('m')) as $key) {
     echo "<li>".getInCatR($key['cat_id']).'<span class="badge bg-dark text-white ml-3">LKR '.$key['amount'].'</span>'."</li>";
   }
...
<?php 
  if (!isset($_GET['year'])) {
    foreach (income(date('Y')) as $v) {
    ?>
        <li class="nav-item  ">
          <a class="nav-link active" href="incomeDay.php?year=<?php 
...
<?php   }} ?>
      ...
      <div class="table-responsive">
        ...
        <table id="user_data" class="table align-items-center table-dark table-flush">
          <thead>
            <tr>
              <th width="10%">name</th>
              <th width="10%" align="center"><span class="float-right">amount</span></th>
            </tr>
          </thead>
          <tbody>

<?php 
//print_r(incomeM(date('Y'),date('m'))) ;

  if (!isset($_GET['year'])) {
    foreach (incomeM(date('Y'),date('m')) as $v) {
    ?>

            <tr>
              <th width="10%"><?php   echo date('Y')."/".date('m')."/".$v['DateV']; ?></th>
              <th width="10%"><span class="float-right"><?php   echo number_format($v['amount'],2,",","."); ?></span></th>
            </tr>
<?php   }
  }else{
foreach (incomeM($_GET['year'],$_GET['day']) as $v) {
    ?>

            <tr>
              <th width="10%"><?php   echo $_GET['year']."/".$_GET['day']."/".$v['DateV']; ?></th>
              <th width="10%"><span class="float-right"><?php   echo number_format($v['amount'],2,",","."); ?></span></th>
            </tr>
<?php   }} ?>
