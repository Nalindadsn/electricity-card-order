...
$stmtIn = $pdo->prepare("SELECT * FROM tone1");
$stmtIn->execute();
...
';
  ...
  $stmtIn2 = $pdo->prepare("SELECT * FROM ttwo1 WHERE tone_id='".$key['tone_id']."'");
  $stmtIn2->execute();
...
if ( pr($prArray[57])==1) {
      echo '
<a  id="'.$key2["ttwo_id"].'" class="   text-warning   update1"> <i class="fas fa-edit"></i></a>

';}

        echo "<ul class='nested'>";

        $stmtIn3 = $pdo->prepare("SELECT * FROM tthree1 WHERE ttwo_id='".$key2['ttwo_id']."'");
        $stmtIn3->execute();
...
if ( pr($prArray[57])==1) {
          echo '
<a  id="'.$key3["tthree_id"].'" class="  text-warning  update2"> <i class="fas fa-edit"></i></a>
...
// $stmt = $conn->prepare("SELECT * from ttwo1WHERE id=?");
